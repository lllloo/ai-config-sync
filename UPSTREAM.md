...
**現況（2026-08-06 重新確認）**：CLI 已到 **1.5.22**，已發布版本**仍無 global scope 的一鍵還原**。上游那份直接解決本 repo 痛點的實作 PR **[#743](https://github.com/vercel-labs/skills/pull/743)（feat: add global restore/relink from .skill-lock.json，Fixes #683，Open 非 draft）** 仍未併入：新增 `experimental_install -g` 讀 `~/.agents/.skill-lock.json`、還原 canonical global skills 並重建 global agent links，並修掉「global restore 誤用 project-scoped agent」的 target-selection bug、附 clean-machine restore 回歸測試——能力正是本 repo 需要的。

**但進度已停滯，且不再是「只差 merge」**：2026-08-06 查 GitHub API，`mergeable_state` 為 **`dirty`**（與 base 有衝突，需作者 rebase），PR 最後更新停在 2026-06-20（約 7 週無動靜），社群留言仍在催進度。原先「只差 merge」的判讀已不成立——現在差的是作者回來解衝突。在 #743 併入前，本 repo `skills-lock.json` 作各裝置 source of truth 的設計依舊有效，`npm run skills:diff` 仍只能提示指令、不自動執行。

**能力盤點（1.5.7 起,下表仍適用於 1.5.22 已發布版）：**
...
| Issue / PR | 狀態 | 最後更新 | 備註 |
|---|---|---|---|
| [#743 feat: add global restore/relink from `.skill-lock.json`](https://github.com/vercel-labs/skills/pull/743) | **Open PR（最對題，已實作，但有衝突）** | 2026-06-20 | **Fixes #683**，非 draft。加 `experimental_install -g` 讀 `~/.agents/.skill-lock.json`、還原 canonical skills + 重建 global agent links，修 target-selection bug、附 clean-machine 回歸測試。**`mergeable_state=dirty`（2026-08-06 查）**：需作者 rebase，非只差 merge |
...
| [#549 `npx skills install`（npm ci 等價）](https://github.com/vercel-labs/skills/issues/549) | Open | 2026-07-15 | `experimental_install` 僅 project；Claude Code symlink 議題已被 #683 接續 |
| [#283 `skills install` / `skills sync` from lock](https://github.com/vercel-labs/skills/issues/283) | Open | 2026-07-08 | 社群 workaround：`npx skills update -p`（慢但能跑） |
...
| [#937 merge skills-npm features into experimental_sync](https://github.com/vercel-labs/skills/pull/937) | Open PR | 2026-06-20 | 相關但非直接解決 |
