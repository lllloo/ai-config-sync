...
- **宣告式同步項目 `SYNC_MANIFEST`**：一列 = 一路徑；可選 `homeLabel` 表示本機檔名不同於 repo `label`，目前無 manifest 使用者，但保留為 materializer 的通用能力，以合成 entry 在 `sync.test.js` 維持正向覆蓋。本機端路徑一律落在 area `homeBase` 之內——**沒有指向 `$HOME` 根目錄的欄位**，另有「manifest 任一列 materialize 後不得指向 `~/.claude.json`」的回歸鎖（比對實際路徑、不綁欄位名）。`fixedFlow` 項目 src/dest 固定不交換，目前只有 `settings.json`，由 `settings` handler 依 direction 決定行為。
