...
// （DEVICE_SETTINGS_KEYS 黑名單 + SENSITIVE_KEY_PATTERN 護欄；env 仍走巢狀白名單）
// -----------------------------------------------------------------------------
...
test('loadStrippedSettings：剝除黑名單 top-level key 與非白名單 env key', () => {
  withTmpDir((dir) => {
...
test('mergeSettingsBetween(to-repo)：寫入 repo 為 stripped 內容且不含金鑰，回傳 true', () => {
  withTmpDir((dir) => {
    ...
    assert.deepEqual(JSON.parse(written), { env: { EDITOR: 'vim' }, permissions: ['a'] },
      'repo 剝除黑名單 top-level key 與非白名單 env key');
  });
});
...
test('值層防線：env 子樹豁免 key 掃描（白名單權威），值掃描仍適用', () => {
  // env 的 key 由 PORTABLE_ENV_KEYS 白名單放行，即使 key 名含敏感字也不因 key 名中止
