...
test('loadStrippedSettings：移除所有 top-level DEVICE_FIELDS 與非白名單 env key', () => {
  withTmpDir((dir) => {
...
test('extractDeviceValues：回傳 top-level DEVICE_FIELDS 及 env 非白名單 key', () => {
  const local = {
    model: 'opus',
    effortLevel: 'high',
    defaultShell: 'powershell',
...
test('extractDeviceValues：local 無 top-level DEVICE_FIELDS 但有非白名單 env 時，萃取該 env 供 to-local 保留', () => {
