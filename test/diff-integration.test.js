...
function setupSandbox() {
  ...
  // 自控 repo 內容：一個 agents package 檔、codex/AGENTS.md、可攜 settings
  writeFile(path.join(repo, 'claude', 'agents', 'pkg', 'sample.md'), 'AGENT-SAMPLE');
...
test('runDiff (to-repo)：本機 HOME 為空 → 回 EXIT_DIFF 並將 repo 檔逐一列為 deleted', () => {
  ...
  try {
    ...
    assert.match(result.stdout, /\[-\] claude\/agents\/pkg\/sample\.md.*repo 有、本機沒有/,
      '應將 repo agents 目錄下的檔案逐一列為 deleted');
