...
3. **產中介表示**：依 `references/intermediate-tables.md` 產四張表（節點、邊、群組、省略）。四表只寫語意、不夾帶圖面決策。節點數**預設壓到 9 以內**，超出就依 `output-spec.md` §3 的 degrade ladder 收斂（合併同質節點、收攏葉群），收不下的寫進省略表；使用者明說要「詳細／完整／faithful」時才放寬到 24，超過 24 一律先畫頂層。大綱頁的 Context 與主題頁的 ER 切片仍守其自身的稀疏定義，不因額度變寬而填滿。
