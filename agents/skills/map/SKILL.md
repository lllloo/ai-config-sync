---
name: map
description: 僅在明確呼叫時啟動——使用者輸入 `/map`，或明講「用 map」「跑 map skill」才執行。即使需求聽起來像架構圖（「給我架構圖」「畫一下架構」「這專案怎麼組起來的」「我剛接手這個 repo」「給我專案大綱」「給我某功能的主題頁」），只要沒點名本 skill，一律不自動啟動；照一般方式回答即可。啟動後聚焦模組關係、依賴方向、資料流與分層約束，依參數產大綱頁、主題頁或指定模組／路徑的聚焦分析，輸出本地 HTML 架構圖。本檔的「產出成頁面」一節仍是本地檔產物（報告、圖表、分析結果）落點與可點連結規則的正典，但只由全域指示的指路句載入、不是本 skill 的觸發訊號。
---
