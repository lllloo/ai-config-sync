...
3. **產中介表示**：依 `references/intermediate-tables.md` 產四張表（節點、邊、群組、省略）；邊表的 `種類` 為必填。四表只寫語意、不夾帶圖面決策。節點數依讀者問題所需決定，**不預先壓到 9**（詳細度由 adapter 依節點數選 detail，見該檔）；超過 24 個才先畫頂層、展不開的區塊寫進省略表。大綱頁的 Context 與主題頁的 ER 切片仍守其自身的稀疏定義，不因額度變寬而填滿。
