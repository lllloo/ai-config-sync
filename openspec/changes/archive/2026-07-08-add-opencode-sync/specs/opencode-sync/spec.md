...
### Requirement: opencode 範本與 init 重置涵蓋

系統 SHALL 為 opencode 設定項目提供 `.example` 骨架範本，且 `npm run init` 的重置流程 SHALL 涵蓋 opencode 項目，使 fork 後可將作者個人資料重置為空骨架。

#### Scenario: init 重置 opencode 設定為骨架

- **WHEN** 使用者於 fork 後執行 `npm run init`
- **THEN** 系統 SHALL 以 `.example` 範本覆寫 opencode 主設定與 `AGENTS.md` 的正式檔
- **AND** `--dry-run` SHALL 能預覽此重置而不實際寫入
