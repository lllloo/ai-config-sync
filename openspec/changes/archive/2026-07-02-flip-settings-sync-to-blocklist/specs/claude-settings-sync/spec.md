...
**Reason**：同步策略反轉——top-level 官方欄位屬有限集合且本身非機密（最高風險為 helper 路徑與裝置偏好），改採「預設同步、黑名單＋敏感 pattern 排除」以消除白名單逐欄追加的維護成本；未知新欄位從「預設留在本機」改為「預設同步（除非命中黑名單或 pattern）」。
**Migration**：由新需求「settings.json top-level 欄位採黑名單混合制同步」取代；`PORTABLE_SETTINGS_KEYS` 常數由 `DEVICE_SETTINGS_KEYS` + `SENSITIVE_KEY_PATTERN` 取代；repo `claude/settings.json` 依新規則重新收斂一次（人工確認新進欄位）。`env` 巢狀白名單不在遷移範圍，維持不動。
