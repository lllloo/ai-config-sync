# bidirectional-sync-workflow Specification

## Purpose
定義本機與 repo 雙向同步的流程契約：`diff` 唯讀且有差異回 `EXIT_DIFF`、`status` 為 `diff`+`skills:diff` 合流、`to-repo` 的 git-repo 前置檢查與完成後顯示 git 狀態、`to-local` 的預覽→確認閘門、非互動環境（非 TTY）拒絕等待而非 hang、`--dry-run` 絕不寫入、內容相同即不寫入的冪等語義。
## Requirements
...
所有同步型別（`file`／`dir`／`settings`／`xtool-dir`）SHALL 一律經過此閘門，其差異 SHALL 計入待寫入變更數；SHALL NOT 存在任何豁免確認流程、或差異不計入變更數的型別。
...
- **WHEN** 本次 `to-local` 存在任一型別（`file`／`dir`／`settings`／`xtool-dir`）的差異
