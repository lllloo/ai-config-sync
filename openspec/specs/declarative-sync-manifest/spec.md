...
`homeLabel` 與 `homeRootFile` 目前皆無 manifest 使用者，SHALL 保留為 materializer 的通用能力並各以合成 entry 維持測試覆蓋。

#### Scenario: 新增同步內容只需加一列

- **WHEN** 維護者要新增一個要同步的檔案或目錄
- **THEN** 只需在 `SYNC_MANIFEST` 增加一列（必填 `area`、`label`、`type`，視需要加上 `homeLabel`／`homeRootFile`／`fixedFlow`／`exclude`）
