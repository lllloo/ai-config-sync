...
- Claude agents 儲存於 `claude/agents/`，以 package 子目錄分組（目前為 `everything-claude-code/`）；Codex 目前無 agent，`codex/agents/` 未列同步項目，日後有需要再於 `SYNC_MANIFEST` 加回（Codex CLI 會遞迴掃描 `~/.codex/agents/` 子目錄）
