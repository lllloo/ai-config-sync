...
- **不同步敏感檔案**：Codex `config.toml`、`~/.claude.json`（含憑證與 MCP 設定）等刻意排除，見 [刻意不同步](#刻意不同步)。
...
- **Agent 定義**以 package 子目錄組織（目前為 `everything-claude-code/`）；Codex 目前無 agent、未列同步項目。
- **Command 定義**：本 repo 已改用 skill、不再新增 command，故目前為空目錄，`sync.js` 仍原樣支援此路徑。
- **全域 Skill 與 `npx skills` 是兩套機制**：`claude/skills/` 是 repo 自帶、Git 版控、由 to-repo／to-local 直接鏡射的 skill；`skills-lock.json` 追蹤的是外部 `npx skills` CLI 安裝的 skill，不受 `sync.js` 管理，只能用 `npm run skills:diff` 比對後手動套用建議。兩者最終都落在 `~/.claude/skills/`，但管理者不同。
- **本地 Skill** 實體放在 `.agents/skills/`（已版控），跨工具共用、不參與 to-repo／to-local；Claude Code 靠 `.claude/skills` symlink 讀取，Codex 原生探索、無需 symlink（見 [刻意不同步](#刻意不同步) 的 Windows 注意）。
...
| 目錄 | 用途 |
|------|------|
| `claude/`、`codex/`、`opencode/`（無點） | **要同步**到各工具全域設定的內容 |
...
`~/.codex/config.toml` 是純本機偏好檔，**刻意不同步**：實質內容只有下列 4 個「設一次就不動」的偏好，為它維護一套過濾／合併機制不划算。新裝置上照下面調整即可，不調也不會壞——只是行為回到 Codex 預設。
...
`npm run safety:check` 是手動、唯讀、離線的檢查，掃描 `claude/`、`codex/`、`opencode/` 與 `skills-lock.json`（不掃 `test/`、`openspec/`、README 等非同步來源）。輸出只列**分類、檔案與欄位／key／行號**，不列 env 值、secret 原值或完整 HOME 路徑。
...
**text pattern 掃描的排除**：secret／私鑰／HOME 路徑的字串掃描會**排除外部套件文件目錄**（`claude/agents/`、`claude/skills/`）。這些是原樣鏡射的第三方文件，為說明偵測規則本就含 token／路徑樣式，掃它們會製造整類誤判。排除只作用於 text 掃描，結構化 `.json`／`.toml` 的 hard block 不受影響。取捨：上游套件文件若真含機密不再被攔屬可接受（公開來源、本 repo 不編輯），真正的機密載體與使用者手改的設定來源仍全覆蓋。
...
- **`~/.codex/config.toml`**（Codex 主設定）— 純本機偏好檔，Codex 不依賴同步。實質內容只有 4 個「設一次就不動」的偏好（見 [Codex 建議設定](#codex-建議設定手動套用)），其餘欄位皆等同官方預設；為此維護 TOML 過濾／序列化／合併機制不划算，改由 README 列建議、手動套用。不同步**不會**造成功能損壞或資料遺失，只是新裝置需手動設一次。`safety:check` 仍對 repo 內任何 `.toml` 保有機密 section 的 hard block 防線。
...
- Claude agents 存於 `claude/agents/`，以 package 子目錄分組（目前 `everything-claude-code/`）；Codex 目前無 agent，`codex/agents/` 未列同步項目，日後有需要再於 `SYNC_MANIFEST` 加回。
