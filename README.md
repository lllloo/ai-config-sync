...
- **不整檔同步敏感設定**：Codex `config.toml` 只接受 `codex/mcp.json` 的受管 MCP section 投影，其餘內容與 OAuth 憑證不進 repo；`~/.claude.json` 永遠排除。見 [刻意不同步](#刻意不同步)。
...
| MCP Server | — | `codex/mcp.json` → `config.toml` 受管 sections | — |
...
- **Codex MCP**：repo `codex/mcp.json` 只保存 Streamable HTTP 的 `transport`／HTTPS `url`／`enabled`；套用時只增修受管 `[mcp_servers.*]`，保留 `config.toml` 其他內容與未受管 MCP。
...
`~/.codex/config.toml` **不做整檔同步**：下列偏好仍由各裝置手動設定；唯一例外是 `codex/mcp.json` 宣告的受管 MCP sections，由同步工具窄範圍合併。新裝置上不調偏好也不會壞——只是行為回到 Codex 預設。
...
- `codex/mcp.json` schema 外欄位、疑似認證欄位、非 HTTPS URL 或格式錯誤；只印欄位路徑不印值
...
- repo source of truth 是 `codex/mcp.json` version 1；目前只接受 Streamable HTTP、HTTPS URL 與 boolean `enabled`。Supermemory 預設全域啟用，不含 `x-sm-project`。
- `to-local` 只重寫 repo/state 識別的受管 MCP sections；所有非 MCP 設定、註解與未受管 MCP 原文保留。受管 section 的單行 `http_headers.Authorization` 視為本機 overlay 原文保留，不參與可攜差異。裝置受管名稱記錄在 `~/.codex/.ai-config-sync-mcp-state.json`，用來清除 repo 已移除的 stale Server，該 state 不進 Git。
- `to-repo` 只讀回 repo 或 state 已識別的受管 Server，不自動吸入本機其他 MCP；本機 Authorization header 會被忽略，未知、重複、額外 header 或無法安全解析的受管欄位仍會 fail closed。
- OAuth、Authorization header、API Key、token、cookie 與登入狀態皆不進 repo、不輸出。Supermemory 採每台裝置本機設定的靜態 Bearer header，不執行 `codex mcp login supermemory`。

#### Supermemory 最終設定流程（每台裝置一次）

本專案使用 Supermemory MCP 4.0 的 API Key 認證，避開 OAuth；不需安裝 `codex-supermemory` Plugin，也不需設定 `export` 或 `launchctl`。真正的 `sm_...` 金鑰只存在各裝置的 `~/.codex/config.toml`，不得放入 `codex/mcp.json`、Git、文件或對話內容。

1. 先執行 `npm run to-local`，讓 repo 的可攜定義建立 Supermemory section。
...
6. 首次呼叫 Supermemory 工具時核准 MCP tool call。先以唯讀的 `whoAmI` 或 `recall` 驗證；只有工具實際成功才算設定完成。不要用 `approval_policy = "never"` 驗證，否則需要核准的 MCP 呼叫會被取消。

同步器只驗證本機 Authorization header 的窄結構並保存完整 raw line；不解析、不正規化、不比較、不輸出其值，也不會寫回 `codex/mcp.json`。因此後續執行 `diff`／`to-local`／`to-repo` 不會洗掉本機 API Key。
...
- **`~/.codex/config.toml` 整檔** — 仍不進 repo、不做整檔同步。偏好、projects、providers、未受管 MCP 與其他 section 保持本機；只有 `codex/mcp.json` 的受管 MCP 透過 section-level merge 投影。`safety:check` 仍對 repo 內任何 `.toml` 的機密 section hard block。
- **`~/.codex/.ai-config-sync-mcp-state.json`** — 裝置本機的受管 MCP 名稱，不進 repo；OAuth／ChatGPT 登入狀態同樣不在同步範圍。
...
| `mcp.js` | Codex MCP manifest 驗證、TOML section 投影與本機受管 state 模組；由 `sync.js` 經 DI 使用 |
...
| `test/mcp.test.js` | MCP schema、TOML 投影、state 與 deterministic serialization 純函式測試 |
