...
跨裝置同步 Claude Code / Codex / opencode 設定的私有 Git repo 工具。一台機器設定好，其他機器一鍵套用。
...
```
本機 ~/.claude ~/.codex ~/.config/opencode
   │  ▲
   │  │ to-local（套用，會先預覽）
   │  │
   ▼  │ to-repo（上傳本機設定）
 私有 Git repo（claude/ codex/ opencode/）──push/clone──▶ 其他裝置
```

- **不寫入本機 MCP 設定**：`~/.claude.json` 與 `~/.codex/config.toml` 皆**永不被本工具寫入**，MCP 改採唯讀偵測 + 輸出官方 CLI 指令。見 [MCP 諮詢式同步](#mcp-諮詢式同步)。
...
| 項目 | Claude Code<br>`~/.claude/` | Codex<br>`~/.codex/` | opencode<br>`~/.config/opencode/` |
|------|------|------|------|
| 全域指示／規則 | `CLAUDE.md` | `AGENTS.md` | `AGENTS.md` |
| 主設定檔 | `settings.json` | —（見 [建議設定](#codex-建議設定手動套用)） | `opencode.jsonc` |
| MCP Server | `claude/mcp.json`（諮詢式，見下） | `codex/mcp.json`（諮詢式，見下） | — |
...
- **全域指示**三者各自獨立；opencode 缺 `AGENTS.md` 時才 fallback 讀 Claude 的 `CLAUDE.md`。
- **主設定檔**：Claude 為黑名單過濾版（排除裝置欄位，見 [settings.json 同步行為](#settingsjson-同步行為)）；opencode 整檔同步，支援 `.json`／`.jsonc` 檔名變體（`.jsonc` 優先）。
- **MCP**：兩端皆為**諮詢式（advisory）**——`diff` 唯讀比對本機、`to-local` 只輸出 `claude mcp add`／`codex mcp add` 指令，不寫入任何本機設定檔。見 [MCP 諮詢式同步](#mcp-諮詢式同步)。
...
| 目錄 | 用途 |
|------|------|
| `claude/`、`codex/`、`opencode/`、`agents/`（無點） | **要同步**到各工具全域設定的內容（`agents/` ↔ `~/.agents/`） |
...
`to-local` 會安裝 Supermemory 的可攜定義；API Key 認證須依下方「Supermemory 最終設定流程」在每台裝置各自設定。成功寫入設定不代表認證或實際 memory／recall 已驗證。
...
`npm run safety:check` 是手動、唯讀、離線的檢查，掃描 `claude/`、`codex/`、`opencode/`、`agents/` 與 `skills-lock.json`（不掃 `test/`、`openspec/`、README 等非同步來源）。輸出只列**分類、檔案與欄位／key／行號**，不列 env 值、secret 原值或完整 HOME 路徑。
...
- `codex/mcp.json`／`claude/mcp.json` schema 外欄位、憑證載體欄位（`headers`／`env` 值）、非 HTTPS URL、URL pathname／query 或 `args` 挾帶憑證、格式錯誤；只印欄位路徑（含 `args[N]` 索引）不印值。判準與同步流程共用同一 validator，不會出現「同步擋下但 safety 放行」
...
兩端 MCP（Claude 與 Codex）採**諮詢式（advisory）**：本工具唯讀偵測差異、輸出官方 CLI 指令，**不寫入任何本機設定檔**。

| | Claude Code | Codex |
|---|---|---|
| repo 來源 | `claude/mcp.json` | `codex/mcp.json` |
| 本機位置（唯讀） | `~/.claude.json` 的 top-level `mcpServers` | `~/.codex/config.toml` 的受管 `[mcp_servers.*]` |
| `to-local` 產出 | `claude mcp add --transport <t> --scope user <name> <url>` | `codex mcp add <name> --url <url>` |

- **為何不自動寫入**：`claude mcp add` 與 `codex mcp add` 都是功能完整的官方 CLI，知道正確的寫入時機、支援各種 transport、處理 OAuth 流程。自行改寫本機設定檔等於重造官方能力，且 `~/.claude.json` 是含 OAuth token 與專案歷史、被 Claude Code 持續寫入的活檔——寫壞的代價是登出與掉歷史，不寫的代價是零。與 `skills:diff` 「只輸出建議指令、不執行」為同一判斷。
- **`to-local` 行為**：不寫檔、不計入待套用筆數、不進確認閘門。若本次差異全部來自 MCP，直接輸出建議指令並結束；使用者在確認提示回答 `n` 時，建議指令仍會輸出（它沒有副作用）。
- **`to-repo` 行為**：由本機讀出可攜身分欄位寫入 repo 來源。`env` 值、`headers`、`Authorization` 一律剝除，不進 repo、不輸出。受管範圍為 repo 已登記的 Server 名。
- **本機額外的 Server**：只在 `diff` 標示，**不產生移除指令、不刪除**。要納入 repo 請跑 `to-repo`（Claude）或手動加進 `codex/mcp.json`（Codex）。
- **憑證 fail closed**：`url` 的 pathname 與 query、`stdio` 的每個 `args` 元素都要通過憑證檢查，判準是「無法判定為安全即拒絕」而非比對已知 token 樣式（path-embedded token 對黑名單免疫）。誤擋合法長路徑是刻意取捨，**不提供繞過旗標**；請改用 `envKeys` 或於本機手動維護。
- **repo 來源只存身分**：Claude 端接受 `type: http`／`sse`／`stdio`、HTTPS `url` 或白名單 `command`＋`args`、諮詢性 `envKeys`（僅 key 名）；Codex 端接受 Streamable HTTP 的 `transport`／HTTPS `url`／`enabled`。無法表達的 transport 一律 fail closed 報錯，絕不靜默丟棄。

#### Supermemory 設定流程（每台裝置一次）
...
接著在 Claude Code 輸入 `/mcp`，選 `supermemory` → `Authenticate` 完成瀏覽器授權。

Codex（API Key）：

1. `codex mcp add supermemory --url https://mcp.supermemory.ai/mcp`
2. 到 [Supermemory API Keys](https://console.supermemory.ai/keys) 建立 `sm_...` 金鑰。
3. 編輯 `~/.codex/config.toml` 中**既有**的 Supermemory section，不要新增同名第二份：

   ```toml
   [mcp_servers.supermemory]
   url = "https://mcp.supermemory.ai/mcp"
   enabled = true
   http_headers = { Authorization = "Bearer sm_你的_API_Key" }
   ```

4. 在 macOS／Linux 執行 `chmod 600 ~/.codex/config.toml`，限制為檔案擁有者可讀寫。這不是加密，但可避免其他本機帳號直接讀取明文金鑰。
5. 完整退出並重開 Codex App；若既有 task 未載入 MCP，建立新 task。
6. 首次呼叫時核准 MCP tool call。先以唯讀的 `whoAmI` 或 `recall` 驗證；只有工具實際成功才算設定完成。不要用 `approval_policy = "never"` 驗證，否則需要核准的 MCP 呼叫會被取消。

真正的 `sm_...` 金鑰只存在各裝置的 `~/.codex/config.toml`，不得放入 `codex/mcp.json`、Git、文件或對話內容。同步器唯讀解析時會**容忍**該 header 存在（不 fail closed），但不讀出、不比較、不輸出其值，也不會寫回 repo；且因為根本不重寫 `config.toml`，後續任何指令都不會洗掉本機 API Key。
...
- **`~/.codex/config.toml`** — 不進 repo，且**永不被本工具寫入**。偏好、projects、providers 與所有 MCP section 皆保持本機；MCP 改由 `to-local` 輸出 `codex mcp add` 指令，由使用者自行執行。`safety:check` 仍對 repo 內任何 `.toml` 的機密 section hard block。
- **`~/.codex/.ai-config-sync-mcp-state.json`** — 舊版投影同步用的受管 MCP 名稱 state，**已不再被讀取或寫入**。從舊版升級的裝置上它是孤兒檔，可自行 `rm`（本工具不會代刪——為了清理而破例寫本機檔會與「不寫入本機」的承諾自相矛盾）。OAuth／ChatGPT 登入狀態同樣不在同步範圍。
- **`~/.claude.json`** — 含 OAuth token、專案級歷史與 MCP 設定，屬高風險敏感活檔，**永不被本工具寫入**。唯一放寬處是唯讀讀取 top-level `mcpServers` 作比對，其餘欄位不讀不解析不輸出。
- **opencode 機密與資料目錄** — opencode 採 XDG 佈局，設定家為 `~/.config/opencode`（非 `~/.opencode`）。機密（`auth.json`）與資料庫（`opencode.db`）落在 `~/.local/share`／`~/.cache`／`~/.local/state`，與設定家分屬不同根目錄，天生不在同步射程；設定家內的執行期產物（`node_modules/`、`package.json`、`package-lock.json`、`plugins/`）因未列入同步清單，也不會被同步。
- **opencode 主設定檔名變體** — 同時接受 `opencode.json` 與 `opencode.jsonc`；同步時以兩端實際存在者為 canonical（`.jsonc` 優先），repo 端恆為單一檔名。若某裝置本機原為 `.json` 而 canonical 解析為 `.jsonc`，`to-local` 會寫入 `.jsonc`，該裝置可能同時留存兩份——請手動刪除非 canonical 的舊檔，避免 opencode 讀到過期設定。
...
各同步內容（`claude/`、`codex/`、`opencode/` 下的檔案）見上方 [同步項目](#同步項目)；此處僅列工具本體。
...
| `mcp.js` | MCP 來源 schema、共用憑證判準、Codex `config.toml` 唯讀解析與 `codex mcp add` 指令產生；由 `sync.js` 經 DI 使用 |
| `claude-mcp.js` | Claude MCP 來源 schema、`~/.claude.json` 唯讀 inspect 與 `claude mcp add` 指令產生；無寫入本機的路徑 |
...
| `test/mcp.test.js` | MCP schema、憑證判準、TOML 唯讀解析與指令產生的純函式測試 |
| `test/claude-mcp.test.js` | Claude MCP schema、唯讀 inspect、指令產生與 handler 零寫入測試 |
